...
describe("Effect HTTP server (wire contract)", () => {
  it("GET /api/meta — shape matches shared Meta", async () => {
    const meta = await waitFor(
      async () => json(await handler(new Request("http://localhost/api/meta")), S.MetaSchema),
      (m) => m.files === 1
    );
    expect(meta).toEqual({
      repoRoot: repoDir,
  ...
  it("GET /api/diff — { files: [...] }", async () => {
    const body = await json(await handler(new Request("http://localhost/api/diff")), S.GetDiffResponseSchema);
  ...
  it("comment CRUD — 201/200/204 + shapes", async () => {
    ...
    // list → { comments: [...] }
    const list = await json(await handler(new Request("http://localhost/api/comments")), S.ListCommentsResponseSchema);
    expect(list.comments).toHaveLength(1);
    expect(list.comments[0]!.id).toBe(comment.id);

    // list with filters → ok
    const open = await json(await handler(new Request("http://localhost/api/comments?status=open")), S.ListCommentsResponseSchema);
  ...
  it("keeps open comments and saved code after commits, including removed code", async () => {
    ...
    const clean = await waitFor(
      async () => json(await handler(new Request("http://localhost/api/diff")), S.GetDiffResponseSchema),
      (diff) => diff.files.length === 0
    );
    expect(clean.files).toEqual([]);
    const list = await json(await handler(new Request("http://localhost/api/comments?status=open")), S.ListCommentsResponseSchema);
    ...
    const recovered = await json(await handler(new Request("http://localhost/api/comments")), S.ListCommentsResponseSchema);
    ...
    const removed = await json(await handler(new Request("http://localhost/api/comments?status=open")), S.ListCommentsResponseSchema);
  ...
  it("isolates recurring code by review and carries comments forward only on explicit request", async () => {
    const diff = async () => json(await handler(new Request("http://localhost/api/diff")), S.GetDiffResponseSchema);
    const list = async () => json(await handler(new Request("http://localhost/api/comments")), S.ListCommentsResponseSchema);
